...
const ProfileRight = (props: Props) => {
  const { jobHired } = useSelector((state:StateType) => state.userReducer)
  const navigate = useNavigate();
  const dispatch:DispatchType = useDispatch();

  const deleteJobHired = (id:number) => {
    dispatch(deleteJobHiredAPI(id))
  }
  ...
  return (
    <div className="profile--right">
      <div className="create--gig">
        ...
        <a href="#">Create a new Gig</a>
      </div>
      <div className="list-job-hired">

        {
          jobHired?.map((job, t) => {
            return <div className="item-hired" key={t}>
            <div className="item-hired-img">
              <img src={job.congViec.hinhAnh} alt="job-img" />
            </div>
            <div className="item-hired-content">
              <div className="item-hired-title">
                <h5>{job.congViec.tenCongViec.length > 90 ? job.congViec.tenCongViec.substring(0,90) + "..." : job.congViec.tenCongViec}</h5>
              </div>
              <div className="item-hired-info">
                {job.congViec.moTaNgan.length > 150 ? job.congViec.moTaNgan.substring(0,150) + '...' : job.congViec.moTaNgan}
              </div>
              <div className="item-hired-star">
                <span className="star"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1792 1792" width="15" height="15"><path fill="#ffbe5b" d="M1728 647q0 22-26 48l-363 354 86 500q1 7 1 20 0 21-10.5 35.5t-30.5 14.5q-19 0-40-12l-449-236-449 236q-22 12-40 12-21 0-31.5-14.5t-10.5-35.5q0-6 2-20l86-500-364-354q-25-27-25-48 0-37 56-46l502-73 225-455q19-41 49-41t49 41l225 455 502 73q56 9 56 46z"></path></svg></span>
                <span className="number-star">{job.congViec.saoCongViec}.0</span>
                <span className="amount-rate">({job.congViec.danhGia})</span>
              </div>
              <div className="item-hired-action">
                <button className="item-hired-detail" onClick={() => {
                  navigate(`/detail/${job.congViec.id}`)
                }}>
                  <i className="fa-solid fa-circle-info"></i>
                </button>
                <button className="item-hired-del" onClick={() => deleteJobHired(job.id)}>
        ...
        {jobHired?.length === 0 &&
        <div className="hired-empty">
          You haven't hired any job yet. 
        </div>
        }
      </div>
    </div>
