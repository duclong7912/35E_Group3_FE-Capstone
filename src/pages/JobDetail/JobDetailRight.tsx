import React from "react";
import { useDispatch, useSelector } from "react-redux";
import { useLocation, useNavigate, useParams } from "react-router-dom";
import {
  JobDetailModel,
  ThueCongViec,
} from "../../Models/jobDetail/jobDetailModel";
import { DispatchType, StateType } from "../../redux/configStore";
import { hireJob } from "../../redux/JobDetailReducer/JobDetailReducer";

type Props = {};

const JobDetailRight = (props: Props) => {
  const location = useLocation();
  const { userLogin } = useSelector((state: StateType) => state.userReducer);
  const { arrJobDetail } = useSelector(
    (state: StateType) => state.JobDetailReducer
  );
  const navigate = useNavigate();
  const param = useParams();
  const dispatch: DispatchType = useDispatch();

  const handleHireJob = () => {
    if (userLogin) {
      const day = new Date();

      const maCongViec = Number(param.id);
      const maNguoiThue = userLogin?.id;
      const today =
        day.getDate() + "/" + (day.getMonth() + 1) + "/" + day.getFullYear();

      const value: ThueCongViec = {
        maCongViec,
        maNguoiThue,
        ngayThue: today,
      };

      const action = hireJob(value);
      dispatch(action);
    } else {
      navigate("/users/login");
    }
  };



  return (
    <div className="jobdetail--right">
      {arrJobDetail.map((detail: JobDetailModel, index: number) => {
        return (
          <div className="content" key={index}>
            <div className="jobdetail--card">
              <div className="choice">
                <div>Basic</div>
                <div>Standard</div>
                <div>Premium</div>
              </div>
              <div className="detail">
                <div className="top">
                  <div>A WEB APP</div>
                  <div>${detail.congViec.giaTien}</div>
                </div>
                <div className="discription">{detail.congViec.tenCongViec}</div>
                <div className="mid">
                  <div>
                    <svg
                      width="16"
                      height="16"
                      viewBox="0 0 16 16"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path d="M8 0C3.6 0 0 3.6 0 8s3.6 8 8 8 8-3.6 8-8-3.6-8-8-8zm0 14c-3.3 0-6-2.7-6-6s2.7-6 6-6 6 2.7 6 6-2.7 6-6 6z"></path>
                      <path d="M9 4H7v5h5V7H9V4z"></path>
                    </svg>
                    2 Days Delivery
                  </div>
                  <div>
                    <svg
                      width="16"
                      height="16"
                      viewBox="0 0 16 16"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path d="M4.50001 11.4999C6.40001 13.3999 9.60001 13.3999 11.5 11.4999C12.2 10.7999 12.7 9.7999 12.9 8.7999L14.9 9.0999C14.7 10.5999 14 11.8999 13 12.8999C10.3 15.5999 5.90001 15.5999 3.10001 12.8999L0.900012 15.0999L0.200012 8.6999L6.60001 9.3999L4.50001 11.4999Z"></path>
                      <path d="M15.8 7.2999L9.40001 6.5999L11.5 4.4999C9.60001 2.5999 6.40001 2.5999 4.50001 4.4999C3.80001 5.1999 3.30001 6.1999 3.10001 7.1999L1.10001 6.8999C1.30001 5.3999 2.00001 4.0999 3.00001 3.0999C4.40001 1.6999 6.10001 1.0999 7.90001 1.0999C9.70001 1.0999 11.5 1.7999 12.8 3.0999L15 0.899902L15.8 7.2999Z"></path>
                    </svg>
                    Unlimited Revisions
                  </div>
                </div>
                <div className="bot">
                  <ul>
                    <li>
                      <svg
                        width="16"
                        height="16"
                        viewBox="0 0 11 9"
                        xmlns="http://www.w3.org/2000/svg"
                        fill="currentFill"
                      >
                        <path d="M3.645 8.102.158 4.615a.536.536 0 0 1 0-.759l.759-.758c.21-.21.549-.21.758 0l2.35 2.349L9.054.416c.21-.21.55-.21.759 0l.758.758c.21.21.21.55 0 .759L4.403 8.102c-.209.21-.549.21-.758 0Z"></path>
                      </svg>
                      1 page
                    </li>
                    <li>
                      <svg
                        width="16"
                        height="16"
                        viewBox="0 0 11 9"
                        xmlns="http://www.w3.org/2000/svg"
                        fill="currentFill"
                      >
                        <path d="M3.645 8.102.158 4.615a.536.536 0 0 1 0-.759l.759-.758c.21-.21.549-.21.758 0l2.35 2.349L9.054.416c.21-.21.55-.21.759 0l.758.758c.21.21.21.55 0 .759L4.403 8.102c-.209.21-.549.21-.758 0Z"></path>
                      </svg>
                      Design customization
                    </li>
                    <li>
                      <svg
                        width="16"
                        height="16"
                        viewBox="0 0 11 9"
                        xmlns="http://www.w3.org/2000/svg"
                        fill="currentFill"
                      >
                        <path d="M3.645 8.102.158 4.615a.536.536 0 0 1 0-.759l.759-.758c.21-.21.549-.21.758 0l2.35 2.349L9.054.416c.21-.21.55-.21.759 0l.758.758c.21.21.21.55 0 .759L4.403 8.102c-.209.21-.549.21-.758 0Z"></path>
                      </svg>
                      Content upload
                    </li>
                    <li>
                      <svg
                        width="16"
                        height="16"
                        viewBox="0 0 11 9"
                        xmlns="http://www.w3.org/2000/svg"
                        fill="currentFill"
                      >
                        <path d="M3.645 8.102.158 4.615a.536.536 0 0 1 0-.759l.759-.758c.21-.21.549-.21.758 0l2.35 2.349L9.054.416c.21-.21.55-.21.759 0l.758.758c.21.21.21.55 0 .759L4.403 8.102c-.209.21-.549.21-.758 0Z"></path>
                      </svg>
                      Responsive design
                    </li>
                    <li>
                      <svg
                        width="16"
                        height="16"
                        viewBox="0 0 11 9"
                        xmlns="http://www.w3.org/2000/svg"
                        fill="currentFill"
                      >
                        <path d="M3.645 8.102.158 4.615a.536.536 0 0 1 0-.759l.759-.758c.21-.21.549-.21.758 0l2.35 2.349L9.054.416c.21-.21.55-.21.759 0l.758.758c.21.21.21.55 0 .759L4.403 8.102c-.209.21-.549.21-.758 0Z"></path>
                      </svg>
                      Source code
                    </li>
                  </ul>
                  {/* {detail.congViec.moTaNgan} */}
                </div>
                <div className="button">
                  <button className="btn" onClick={handleHireJob}>
                    Continue (${detail.congViec.giaTien})
                  </button>
                  <button className="btn">Compare Packages</button>
                </div>
              </div>
            </div>
            <div className="contact">
              <div className="contact--content ">
                Contact Seller
                <svg
                  width="14"
                  height="9"
                  viewBox="0 0 14 9"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path d="M0.190662 1.2721L0.809349 0.653381C0.955787 0.506943 1.19322 0.506943 1.33969 0.653381L7.00001 6.30022L12.6603 0.653381C12.8068 0.506944 13.0442 0.506944 13.1907 0.653381L13.8094 1.2721C13.9558 1.41854 13.9558 1.65597 13.8094 1.80244L7.26519 8.34663C7.11875 8.49307 6.88132 8.49307 6.73485 8.34663L0.190662 1.80244C0.0441933 1.65597 0.0441933 1.41854 0.190662 1.2721Z"></path>
                </svg>
              </div>
            </div>
          </div>
        );
      })}
    </div>
  );
};

export default JobDetailRight;
